...
window.renderGrid = function() {
  ...
  filtered.forEach(function(it, i) {
    ...
    card.innerHTML =
      ...
      '<button class="fav-btn' + (isFav ? ' active' : '') + '" aria-label="أضف للمفضلة"><svg viewBox="0 0 24 24" fill="' + (isFav ? 'currentColor' : 'none') + '" stroke="currentColor" stroke-width="2"><path d="M12 21s-7-4.35-9.5-8.5C.7 9 2 5.5 5.5 5c2-.3 3.6.7 4.5 2.2C10.9 5.7 12.5 4.7 14.5 5c3.5.5 4.8 4 3 7.5C19 16.65 12 21 12 21z"></path></svg></button>' +
      imgContent +
      '</div>' +
...
// === Firestore Listener (Read Only) ===
const q = window.query(window.itemsCol, window.orderBy('createdAt', 'desc'));
window.onSnapshot(q, (snapshot) => {
  ...
  document.getElementById('grid').innerHTML = '<div class="empty-state">تعذّر تحميل السلع. تحقق من اتصالك بالإنترنت.</div>';
});
